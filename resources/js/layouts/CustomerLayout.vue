<template>
  <div class="storefront-layout" :class="{ 'storefront-layout--announcement-sticky': isAnnouncementSticky && !isScrolled, 'storefront-layout--scrolled': isScrolled }">
    ...
    <a 
      href="https://wa.me/919944285102" 
      target="_blank" 
      class="whatsapp-floating-bubble-global" 
      :class="{ 'whatsapp-bubble-on-pdp': isProductDetailPage }"
...
const isProductDetailPage = computed(() => {
  return route.name === 'product-detail' || (route.path && route.path.includes('/products/'));
});
...
@media (max-width: 767px) {
  ...
  .whatsapp-floating-bubble-global {
    bottom: 80px; /* Positions it nicely above the 64px bottom nav on normal pages */
    right: 16px;
    width: 54px; /* Touch-friendly and large on mobile */
    height: 54px;
    box-shadow: 0 6px 20px rgba(37, 211, 102, 0.25);
    transition: bottom 0.3s cubic-bezier(0.4, 0, 0.2, 1), transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1), background-color 0.3s, box-shadow 0.3s;
  }
  .whatsapp-floating-bubble-global.whatsapp-bubble-on-pdp {
    bottom: 140px; /* Elevated on Product Detail page so it never covers the sticky bottom Add to Cart bar */
  }
  .whatsapp-bubble-icon {
    width: 28px;
    height: 28px;
